...
def replace_demo_in_cmake(target_dir, new_name, old_name='demo'):
...
def main():
    ...
    parser.add_argument("type", choices=['c', 'q', 'm'], help="Project type: c(cpp) or q(qt) or m(maxsim)")
    ...
    source_map = {
        ...
        'm': os.path.join(base_path, "maxsim", "maxsim-demo-agent")
    }
    old_name = 'demo-agent' if args.type == 'm' else 'demo'
    
    target_parent = os.path.join(base_path, "cpp" if args.type == 'c' else "qt" if args.type == 'q' else "maxsim")
    ...
    replace_demo_in_cmake(target_dir, args.target_name, old_name)
